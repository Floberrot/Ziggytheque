...
final class Version20260519000000 extends AbstractMigration
{
    ...
    public function up(Schema $schema): void
    {
        ...
        $this->addSql(<<<'SQL'
            COMMENT ON COLUMN auth_tokens.expires_at IS '(DC2Type:datetime_immutable)'
        SQL);

        $this->addSql(<<<'SQL'
            COMMENT ON COLUMN auth_tokens.created_at IS '(DC2Type:datetime_immutable)'
        SQL);

        $this->addSql(<<<'SQL'
            COMMENT ON COLUMN auth_tokens.consumed_at IS '(DC2Type:datetime_immutable)'
        SQL);

        // collection_entries: replace unique(manga_id) with unique(owner_id, manga_id), add FK
        $this->addSql(<<<'SQL'
